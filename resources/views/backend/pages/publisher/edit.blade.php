...
@section('content')
<!-- partial -->
<h4 style="text-align: center">Edit publisher</h4>
<form action="{{ route('admin.publisher.update', $publisher_edit->id) }}" method="post" enctype="multipart/form-data">
    @csrf
    <div class="form">
        <div class="form-group">
            <label for="name">Title</label>
            <input type="name" value="{{$publisher_edit->name}}" class="form-control col-md-6" id="name" name="name"
                placeholder="publisher title">
        </div>
        <div class="form-group">
            <label for="description">Description</label>
            <textarea class="form-control" id="description" name="description"
                rows="3">{{$publisher_edit->description}}</textarea>
        </div>
        <div class="form-row">
            <div class="form-group col-md-6">
                <label for="description">Old Image</label>
                @if($publisher_edit->image == null)
                    <img class="mt-3" src="{!!asset('images/default/publisher.png')!!}" width="250px"
                        height="200px" />
                @else 
                <img class="mt-3" src="{!!asset($publisher_edit->image)!!}" width="250px"
                height="200px" />
                @endif
    ...
    <input type="submit" class="btn btn-primary" value="Save publisher">
</form>
